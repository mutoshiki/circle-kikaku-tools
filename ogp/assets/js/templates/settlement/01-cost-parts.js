...
(function () {
  ...
  function formatPaymentBadge(label = '支払い') {
    ...
    return `<div class="seisan-car-summary-payment seisan-cost-total-row" aria-label="運転手への支払い金額"><span class="seisan-cost-total-label">合計</span>${formatPaymentBadge('支払い')}<strong class="seisan-car-summary-total ${UI_CLASS.amount}"><span class="seisan-amount-sign" aria-hidden="true">＝</span>${money(amount, helpers)}</strong></div>`;
  }
