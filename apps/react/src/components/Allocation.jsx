...
export default function Allocation({ runtime, room, type, onNotice, onParticipants }) {
  ...
  function personRow(person, inWaiting = false) {
    ...
    return <ContainedListItem key={id} action={<div className="person-actions">
      ...
      <OverflowMenu ref={node => node ? personMenus.current.set(id, node) : personMenus.current.delete(id)} onOpen={() => trackMenuKeys(id)} onClose={() => closeMenu(id)} ariaLabel={`${record.name}の操作`} iconDescription={`${record.name}の操作`} size={isMobile ? 'lg' : 'sm'} flipped>
  ...
  if (!count) return <section className="allocation-page"><div className="empty-state"><h1>{type === 'team' ? '班割' : '車割'}</h1><p>参加者がいません</p><Button onClick={onParticipants}>参加者を追加</Button></div></section>;
  return <section className="allocation-page" aria-label={type === 'team' ? '班割' : '車割'}>
    <div className="allocation-toolbar"><div className="allocation-toolbar-summary">{type === 'team' && <h1>班割</h1>}<p><span>未割り当て <strong>{projection.waiting.length}人</strong></span><span className="row-detail">{count}人・{projection.cars.length}{type === 'team' ? '班' : '台'}</span></p></div>
