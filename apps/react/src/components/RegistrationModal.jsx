...
export default function RegistrationModal({ runtime, onClose, onNotice }) {
  ...
  function submit() {
    ...
    if (parsed && !parsed.ok) { setError(parsed.errors.join(' ')); return; }
    if (!people.length) { setError('参加者を入力してください。'); return; }
  ...
  return <Modal className="app-modal registration-modal" open hasScrollingContent modalHeading="参加者登録" size="lg" primaryButtonText="登録" secondaryButtonText="キャンセル" onRequestSubmit={submit} onRequestClose={onClose} preventCloseOnClickOutside selectorPrimaryFocus="#registration-sheet">
    <div className="form-stack">
      ...
      <TextArea id="registration-sheet" labelText="Googleフォームの回答を貼り付け" helperText="各項目の見出しの行も必ず一緒にコピーしてください。" value={sheet} onChange={event => setSheet(event.target.value)} rows={4} />
      ...
      <div className="form-grid">
        <TextArea id="registration-members" labelText="参加者（改行区切り）" value={members} onChange={event => setMembers(event.target.value)} disabled={!!sheet.trim()} />
        <TextArea id="registration-drivers" labelText="車出し可能な参加者" value={drivers} onChange={event => setDrivers(event.target.value)} disabled={!!sheet.trim()} />
