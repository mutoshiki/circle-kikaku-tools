  ...
  return <Modal className="app-modal registration-modal" open hasScrollingContent modalHeading="参加者登録" size="lg" primaryButtonText="登録" secondaryButtonText="キャンセル" onRequestSubmit={submit} onRequestClose={onClose} preventCloseOnClickOutside selectorPrimaryFocus="#registration-sheet">
    <div className="form-stack">
      ...
      <div className="form-grid">
        <TextArea id="registration-members" labelText="参加者（改行区切り）" value={members} onChange={event => updatePrimaryInput(setMembers, event.target.value)} disabled={!!sheet.trim()} />
        <TextArea id="registration-drivers" labelText="車出し可能な参加者" value={drivers} onChange={event => updatePrimaryInput(setDrivers, event.target.value)} disabled={!!sheet.trim()} />
