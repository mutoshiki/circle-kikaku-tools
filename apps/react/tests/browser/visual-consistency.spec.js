...
test('five-screen Carbon consistency at mobile width', async ({ page }) => {
  ...
  const firstGroup = page.locator('.allocation-group').first();
  const removeGroup = page.getByRole('button', { name: '削除', exact: true }).first();
  await expect(removeGroup).toBeVisible();
  await expect(removeGroup).toHaveClass(/cds--btn--danger/);
  ...
  const paidCheckbox = paymentCard.getByRole('checkbox', { name: '支払い済みにする', exact: true });
  await paidCheckbox.check({ force: true });
  await expect(paidCheckbox).toBeChecked();
  await expect(paymentCard.getByText('支払い済み', { exact: true })).toBeVisible();
  await paidCheckbox.uncheck({ force: true });
