...
test('live PlaceAutocompleteElement selection triggers the integrated route workflow', async ({ page }) => {
  ...
  await page.waitForSelector('#routeOriginAutocomplete gmp-place-autocomplete');
  ...
  await choose('#routeOriginAutocomplete gmp-place-autocomplete', '信州大学 長野');
  await expect.poll(() => page.evaluate(() => ensureSettlementState().routePlanner.origin?.placeId || '')).not.toBe('');
  await choose('#routeDestinationAutocomplete gmp-place-autocomplete', '松本駅');
