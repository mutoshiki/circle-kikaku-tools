...
assert.match(route, /waypoint-keyboard-reordered/, 'waypoints must support keyboard reordering');
...
for (const id of ['routeOriginAutocomplete','routePlannerRetryBtn','routeWaypointList','routeDestinationAutocomplete','routeMap','routeCandidateList','applyRouteDistanceBtn']) assert.match(modal, new RegExp(`id="${id}"`));
