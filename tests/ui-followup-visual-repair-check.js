...
assert(/\.seisan-cost-policy-tag\.split,[\s\S]*?--settlement-tag-bg:\s*var\(--settlement-split-bg\)/.test(summaryCss), 'summary split badge should keep split background token');
assert(/\.seisan-cost-policy-tag\.split,[\s\S]*?--settlement-tag-line:\s*var\(--settlement-split-line\)/.test(summaryCss), 'summary split badge should keep split border token');
