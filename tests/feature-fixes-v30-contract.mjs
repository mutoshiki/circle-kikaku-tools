...
const carCostTemplates = read('assets/js/templates/settlement/03-car-cost-templates.js');
...
expect(collectionTemplate.includes('seisan-check-item--system') && collectionTemplate.includes('集金不要') && collectionTemplate.includes('支払額から差し引き済み') && collectionTemplate.includes('企画者のため集金対象外'), 'Derived collection states must use explicit neutral no-collection rows');
expect(collectionCss.includes('var(--cds-text-secondary') && !collectionCss.includes('seisan-check-item--system.pre-deducted .seisan-check-status') && !collectionCss.includes('var(--cds-link-primary'), 'No-collection checklist statuses must use Carbon secondary text instead of link color');
