...
assert.match(uiSource, /kind === 'error'[\s\S]*showSyncToast\('error', nextMessage, \{ persistent: true \}\)/, 'save or sync errors remain visible until replaced or dismissed');
assert.match(uiSource, /kind === 'connected'[\s\S]*previousVisible[\s\S]*showSyncToast\('connected', nextMessage\)/, 'completion is announced only when prior progress or recovery made it meaningful');
...
assert.match(notificationCss, /\.app-status-toast[\s\S]*width:\s*min\(24rem, 100%\)/, 'toast sizing is owned by the notification region instead of the project-title slot');
