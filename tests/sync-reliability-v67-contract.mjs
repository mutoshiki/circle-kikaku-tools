...
assert.match(roomStatusCss, /\.sync-status-badge\s*\{[\s\S]*position:\s*absolute;[\s\S]*top:\s*50%;[\s\S]*left:\s*50%;[\s\S]*width:\s*fit-content;/, 'sync status must remain a compact overlay in the shell title slot');
assert.match(roomStatusCss, /@media \(max-width: 768px\)[\s\S]*\.sync-status-badge\s*\{\s*width:\s*fit-content;\s*max-width:\s*calc\(100% - 16px\);\s*\}[\s\S]*\.sync-status-badge\.is-visible/, 'mobile status must retain the compact visible-state contract at the app shell breakpoint');
