...
assert.match(uiSource, /if \(!changed && !\(kind === 'connected' && \(state\.syncSavePending \|\| state\.syncCurrentSaveSuppressed\)\)\) return;/, 'a pending real save or explicitly quiet save may close even when status text repeats');
assert.match(uiSource, /syncSavePending:\s*false/, 'real save completion has an explicit notification-cycle owner');
assert.match(uiSource, /syncSuppressNextSaveCycle:\s*false[\s\S]*syncCurrentSaveSuppressed:\s*false/, 'quiet interactions own exactly one candidate save cycle instead of a broad time window');
...
assert.match(uiSource, /suppressThisCycle = state\.syncSuppressNextSaveCycle[\s\S]*state\.syncSuppressNextSaveCycle = false[\s\S]*state\.syncCurrentSaveSuppressed = true/, 'quiet-interaction suppression is consumed at save start');
assert.match(uiSource, /kind === 'connected' && state\.syncCurrentSaveSuppressed[\s\S]*state\.syncCurrentSaveSuppressed = false/, 'the quiet save cycle releases suppression as soon as it completes');
