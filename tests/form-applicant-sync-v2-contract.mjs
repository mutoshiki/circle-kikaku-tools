...
assert.match(announcement, /id=\"announcementOpening\"[\s\S]*?placeholder=\"例：ご応募ありがとうございました！\"/);
assert.match(announcement, /id=\"announcementWeather\"[\s\S]*?placeholder=\"例：雨天の場合は前日20時までに連絡します\"/);
assert.match(announcement, /id=\"announcementNotes\"[\s\S]*?placeholder=\"例：防寒着と飲み物を忘れずに持参してください\"/);
assert.match(announcement, /id=\"announcementContact\"[\s\S]*?placeholder=\"例：山田\"/);
...
assert.match(announcement, /placeholder=\"例：登山口到着・登山開始\"/);
