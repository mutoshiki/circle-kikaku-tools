    ...
    return `<div class="seisan-car-row ${UI_CLASS.surfaceCard}${rowClass}" data-driver-name="${esc(car.name, helpers)}"${standaloneData}>
        ...
        <div class="seisan-car-inputs">
          ...
          <div class="seisan-gas-field-row" role="group" aria-label="ガソリン代の計算条件">
            <label class="seisan-distance-field"><span class="seisan-mini-label">移動距離（km）</span><cds-number-input size="lg" inputmode="decimal" data-field="dist" class="${UI_CLASS.input} ${fieldErrorClass(issues, car.name, 'dist')}" value="${esc(cState.dist || '', helpers)}" label="移動距離（km）" hide-label hide-steppers></cds-number-input></label>
            <label class="seisan-fuel-field"><span class="seisan-mini-label">燃費（km/L）</span><cds-number-input size="lg" inputmode="decimal" data-field="eco" class="${UI_CLASS.input} ${fieldErrorClass(issues, car.name, 'eco')}" value="${esc(cState.eco || '', helpers)}" label="燃費（km/L）" hide-label hide-steppers></cds-number-input></label>
            <label class="seisan-fuel-field"><span class="seisan-mini-label">ガソリン単価（円/L）</span><cds-number-input size="lg" inputmode="decimal" data-field="price" class="${UI_CLASS.input} ${fieldErrorClass(issues, car.name, 'price')}" value="${esc(cState.price || '', helpers)}" label="ガソリン単価（円/L）" hide-label hide-steppers></cds-number-input></label>
          </div>
