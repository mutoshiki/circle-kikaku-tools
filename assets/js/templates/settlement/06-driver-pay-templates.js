  ...
  function driverPay({ result, state, helpers = {} }) {
    ...
    return driverCars.map(car => {
      ...
      return `<div class="seisan-driver-pay-row ${done ? 'done' : ''}" data-carbon-checkbox-row>
            ...
            <cds-checkbox ${done ? 'checked' : ''} data-settlement-driver-paid-name="${encodeURIComponent(car.name)}" label-text="" aria-label="${esc(driverLabel, helpers)}への支払いチェック"></cds-checkbox>
