...
function setRouteHelperStatus(message, isError = false) {
    ...
    const title = byId('routeHelperStatusTitle');
    const subtitle = byId('routeHelperStatusMessage');
    if (!el || !title || !subtitle) return;
    const text = String(message || '').trim();
    el.hidden = !text;
    el.kind = isError ? 'error' : 'info';
    el.setAttribute('kind', el.kind);
    title.textContent = isError ? '入力内容を確認してください' : '移動距離計算ツール';
