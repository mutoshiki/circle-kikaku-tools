...
const report = { generatedAt: new Date().toISOString(), browser: '', screenshots: [], console: [], requestFailures: [], metrics: [] };
...
async function captureScenario(browser, size) {
  const dpr = 1;
  const context = await browser.newContext({ viewport: { width: size.width, height: size.height }, deviceScaleFactor: dpr, colorScheme: 'light', reducedMotion: 'reduce', locale: 'ja-JP' });
  ...
  if (key) {
    ...
    await openHeaderMenu(page); await page.locator('#globalGuideBtn').click();
    await page.locator('#globalGuideModal').waitFor({ state: 'visible' });
    await shot(page, 'global-guide-modal', size, '#globalGuideModal .modal-body', ['top', 'bottom'], dpr);
    await closeAllOverlays(page);
