  ...
  try {
    ...
    try {
      for (const width of [320, 390, 430]) {
        ...
        await setup(page, `CAPTURE-V4-${width}`);
        await page.locator('.seisan-car-panel').screenshot({ path: `/mnt/data/circle-settlement-compact-${width}-v4.png` });
        if (width === 390) {
          await page.locator('#seisanCarLayoutToggle').click();
          await page.waitForTimeout(100);
          await page.locator('.seisan-car-panel').screenshot({ path: '/mnt/data/circle-settlement-list-390-v4.png' });
        }
        await page.close();
      }
    } finally {
